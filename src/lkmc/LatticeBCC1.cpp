...
LatticeBCC1::LatticeBCC1(Kernel::Domain *p, const LatticeParam *lp, Kernel::M_TYPE mt)
 : Lattice(p, lp, mt)
{

}

LatticeBCC1::~LatticeBCC1() {
}

void LatticeBCC1::fill(const Kernel::Coordinates &mc, const Kernel::Coordinates &Mc,
	...
	for(int i=m[0]; i<=M[0]; ++i)
		for(int j=m[1]; j<=M[1]; ++j)
			for(int k=m[2]; k<=M[2]; ++k)
			{
				...
				if(onlyFrame)
					continue;
				tryInBox(pos, mc, Mc, 1, 0, toRotated(Coordinates(a*(i+.50), a*(j+.50), a*(k+.50))));
			}
}
