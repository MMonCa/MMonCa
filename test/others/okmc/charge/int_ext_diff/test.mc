...
test tag=$T.$conc.hole array={[extract electrostatic  name=hole.density dimension=1]} value=$conc error=.35 init=5 end=$size
test tag=$T.$conc.CBiM array={[extract profile.mobile name=BI state=- dimension=1]} value=$CBiM   error=.40 init=5 end=$size
