...
for { set n 0 } { $n < $C_number } { incr n } {
	insert coord={ [expr rand()*$sizeX] [expr rand()*$sizeYZ] [expr rand()*$sizeYZ] } particle=C no.print
}
...
test tag=$T.I array={ [extract profile.mobile name=I] } value=$CI init=1 end=$sizeX error=.15
...
set convFactor [expr 6e14/($l*$l)]
set v_cap [expr 4./3.*3.14159*$l*$l*$l*0.98*0.98*0.98*1e-21]
set CCi   [expr $CC*$Cif_P*exp(($Cf_E - $Cif_E)/($kB*($T+273.15)))]
