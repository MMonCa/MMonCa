...
set sizeZ    [expr sqrt(2.)*.5431*20]
set sizeY    [expr sqrt(2.)*.5431*36]
...
set orig_depth [lindex [extract ac.mean min.y=0 max.y=$sizeY min.z=0 max.z=$sizeZ] 0]
lowmsg "Original depth is $orig_depth"

anneal time=1 temp=$T depth=41
set end_time [extract time]
set end_depth [lindex [extract ac.mean min.y=0 max.y=$sizeY min.z=0 max.z=$sizeZ] 0] 
